...
// enabled.
//
// Deliberately not in an anonymous namespace: HorizontalTabStyleViews
// befriends this class by name (see the `add_friend` substitution in
// horizontal_tab_style_views.h.yaml) to expose the private virtuals this
// class overrides and calls into. That friend declaration only resolves to a
// class with external linkage; a same-named class in an anonymous namespace
// is a distinct type as far as the friendship is concerned, so its access to
// the base's private members would silently fail to apply.
