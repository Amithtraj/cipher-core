...
// streamable event should be appended to.
// Rough guide is that anything that's not generated in the single thread of LLM
// thought should be non-splitting. Anything the server would generate in a
// separate task should be non-splitting. Anything that's generated by the main
// LLM thread should be appended, if it's a streamable type.
bool IsNonSplittingEvent(
  ...
  switch (event_tag) {
    ...
    // previous non-splitting event is not of the same type.
    case mojom::ConversationEntryEvent::Data_::ConversationEntryEvent_Tag::
        kCompletionEvent:
    case mojom::ConversationEntryEvent::Data_::ConversationEntryEvent_Tag::
        kToolUseEvent:
...
mojom::ConversationEntryEvent* MaybeGetEventToAppend(
...
void ConversationHandler::UpdateOrCreateLastAssistantEntry(
  ...
  if (event) {
    if (event->is_completion_event()) {
      ...
      auto* existing_completion =
          MaybeGetEventToAppend(event.get(), *entry->events);
    ...
    if (event->is_tool_use_event() && entry->events->size() > 0) {
      ...
      // previous event.
      auto* last_event = MaybeGetEventToAppend(event.get(), *entry->events);
