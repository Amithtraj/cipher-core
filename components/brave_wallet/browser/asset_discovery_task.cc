...
void AssetDiscoveryTask::DiscoverAssets(
  ...
  DiscoverPolkadotAssetsFromRegistry(
      ...
      FilterByCoin(fungible_chain_ids, mojom::CoinType::DOT), barrier_callback);
...
void AssetDiscoveryTask::OnGetPolkadotTokenRegistry(
  ...
  LOG(INFO) << "Found some asset ids: "
            << base::JoinString(
                   base::ToVector(
                       asset_ids,
                       [](uint32_t id) { return base::NumberToString(id); }),
                   ", ");
...
void AssetDiscoveryTask::MergeDiscoveredPolkadotAssets(
  ...
  for (const auto& tokens : discovered_assets) {
    for (const auto& token : tokens) {
      DCHECK(token->visible);
      LOG(INFO) << "Discovered the following asset: " << token->chain_id << ":"
                << token->name;
      // Several accounts can hold the same asset; AddUserAsset dedupes.
      if (!AddUserAsset(prefs_, token.Clone())) {
        continue;
      }
      discovered_tokens.push_back(token.Clone());
